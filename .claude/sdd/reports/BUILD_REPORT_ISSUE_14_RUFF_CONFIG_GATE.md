...
| E501 (×1) | `bll/web.py:634` | Wrapped the SSE `yield f"..."` in parens, split at the natural `\n` boundary between the "event:" and "data:" lines | Adjacent string-literal concatenation is compile-time identical to one literal; `ruff format` re-joined it into one literal (fits at the new indent) — same expression either way |
